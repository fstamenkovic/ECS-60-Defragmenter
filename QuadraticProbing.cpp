         ...
         */
        template <class HashedObj>
        void QuadraticHashTable<HashedObj>::insert( unsigned index, const HashedObj & x )
        {
                // Insert x as active
            int currentPos = findPos( index );
            if( isActive( currentPos ) )
                return;
            array[ currentPos ] = HashEntry( index, x, ACTIVE );
        ...
        void QuadraticHashTable<HashedObj>::makeEmpty( )
        {
          ...
          for( int i = 0; i < array.size( ); i++ )
            array[ i ].info = EMPTY;
        } //make it empty in the beggining, constructor
         ...
         */
        template <class HashedObj>
        int QuadraticHashTable<HashedObj>::findPos( unsigned index ) const
        {
...
/* 3*/      while( array[ currentPos ].info != EMPTY &&
            ...
            {
              currentPos++; //edited by Filip, this should make it Linear Probing Hash
              collisionNum++; //edited by Filip
...
/* 7*/      return currentPos;
         ...
         */
        template <class HashedObj>
        void QuadraticHashTable<HashedObj>::remove( unsigned index )
        {
            int currentPos = findPos( index );
            if( isActive( currentPos ) )
         ...
         */
        template <class HashedObj>
        const HashedObj & QuadraticHashTable<HashedObj>::find( unsigned index ) const
        {
            int currentPos = findPos( index );
